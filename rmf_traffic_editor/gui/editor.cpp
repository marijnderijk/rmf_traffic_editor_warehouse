...
Editor::Editor() : QMainWindow() {
  ...
  connect(layer_table, &QTableWidget::cellClicked, [&](int row, int /*col*/) {
    if (row - 1 < static_cast<int>(building.levels[level_idx].layers.size())) {
      layer_idx = row;
  ...
  connect(layer_table, &LayerTable::redraw_scene, this,
          &Editor::layer_table_update_slot);

  connect(layer_table, &LayerTable::add_button_clicked, this,
          &Editor::layer_add_button_clicked);

  connect(layer_table, &LayerTable::edit_button_clicked, this,
          &Editor::layer_edit_button_clicked);

  level_table = new LevelTable;
  connect(level_table, &QTableWidget::cellClicked, [&](int row, int /*col*/) {
    if (row < static_cast<int>(building.levels.size())) {
      // save the center point of the current level's image coordinates
      const QPoint p_center_window(map_view->viewport()->width() / 2,
                                   map_view->viewport()->height() / 2);
      QPointF p_center_scene = map_view->mapToScene(p_center_window);

      QPointF p_transformed;
      building.transform_between_levels(level_idx, p_center_scene, row,
                                        p_transformed);
  ...
  connect(level_table, &LevelTable::redraw_scene, this,
          &Editor::level_table_update_slot);

  lift_table = new LiftTable;
  connect(lift_table, &TableList::redraw, [this]() { this->create_scene(); });

  traffic_table = new TrafficTable;
  connect(traffic_table, &TableList::redraw,
          [this]() { this->create_scene(); });

  connect(traffic_table, &QTableWidget::cellClicked, [&](int row, int /*col*/) {
  ...
  connect(crowd_sim_table, &QTableWidget::cellClicked, [&]() {
  ...
  property_editor->setStyleSheet(
      "QTableWidget { background-color: #e0e0e0; color: black; gridline-color: "
      "#606060; } QLineEdit { background:white; }");
  property_editor->setMinimumSize(600, 200);
  property_editor->setSizePolicy(QSizePolicy::Fixed,
  ...
  connect(property_editor, &QTableWidget::cellChanged, this,
          &Editor::property_editor_cell_changed);
  ...
  connect(add_param_button, &QAbstractButton::clicked, this,
          &Editor::add_param_button_clicked);

  delete_param_button = new QPushButton("Delete property");
  delete_param_button->setEnabled(false);
  connect(delete_param_button, &QAbstractButton::clicked, this,
          &Editor::delete_param_button_clicked);
  ...
  building_menu->addAction("&New...", this, &Editor::building_new,
                           QKeySequence(Qt::CTRL + Qt::Key_N));

  building_menu->addAction("&Open...", this, &Editor::building_open,
                           QKeySequence(Qt::CTRL + Qt::Key_O));

  building_menu->addAction("&Save", this, &Editor::building_save,
                           QKeySequence(Qt::CTRL + Qt::Key_S));

  building_menu->addAction("&Export layer alignment points for level", this,
                           &Editor::building_export_features,
                           QKeySequence(Qt::CTRL + Qt::Key_E));

  building_menu->addSeparator();

  building_menu->addAction("E&xit", this, &QWidget::close,
                           QKeySequence(Qt::CTRL + Qt::Key_Q));

  // EDIT MENU
  QMenu *edit_menu = menuBar()->addMenu("&Edit");
  edit_menu->addAction("&Undo", this, &Editor::edit_undo, QKeySequence::Undo);
  edit_menu->addAction("&Redo", this, &Editor::edit_redo, QKeySequence::Redo);
  edit_menu->addSeparator();

  edit_menu->addAction("&Building properties...", this,
                       &Editor::edit_building_properties);
  edit_menu->addSeparator();

  edit_menu->addAction("Rotate all models...", this,
                       &Editor::edit_rotate_all_models);
  edit_menu->addSeparator();

  edit_menu->addAction("Optimize layer transforms", this,
                       &Editor::edit_optimize_layer_transforms,
                       QKeySequence(Qt::CTRL + Qt::Key_T));
  edit_menu->addSeparator();

  edit_menu->addAction("Update storage racks", this,
                       &Editor::update_storage_racks,
                       QKeySequence(Qt::CTRL + Qt::Key_U));
  edit_menu->addSeparator();

  edit_menu->addAction("Align &colinear", this, &Editor::edit_align_colinear,
  ...
  create_tool_button(TOOL_ADD_CONSTRAINT, ":icons/constraint.svg",
                     "Add Constraint");
  create_tool_button(TOOL_ADD_FIDUCIAL, ":icons/fiducial.svg",
  ...
  create_tool_button(TOOL_ADD_MEAS, ":icons/measurement.svg",
                     "Add Measurement (T)");
  ...
  toolbar->setStyleSheet(
      "QToolBar {background-color: #404040; border: none; spacing: 5px} "
      "QToolButton {background-color: #c0c0c0; color: blue; border: 1px solid "
      "black;} QToolButton:checked {background-color: #808080; color: red; "
  ...
  const int left = settings.contains(preferences_keys::window_left)
                       ? settings.value(preferences_keys::window_left).toInt()
                       : 0;

  const int top = settings.contains(preferences_keys::window_top)
  ...
  connect(cache_size_update_timer, &QTimer::timeout, this,
...
void Editor::load_model_names() {
  ...
  if (thumbnail_path.isEmpty()) {
    ...
    try {
      share_dir = ament_index_cpp::get_package_share_directory(
          "rmf_traffic_editor_assets");

      ament_index_cpp::get_resource("rmf_traffic_editor_assets", "assets",
                                    assets_dir);
    ...
    thumbnail_path = QDir::cleanPath(
        QDir(QApplication::applicationDirPath())
            .filePath((share_dir + "/" + assets_dir + "/thumbnails").c_str()));
  ...
  for (YAML::const_iterator it = ym.begin(); it != ym.end(); ++it)
    editor_models.emplace_back(it->as<std::string>(), model_meters_per_pixel);
}

QToolButton *Editor::create_tool_button(const ToolId id,
...
Editor *Editor::get_instance() { return instance; }
...
bool Editor::load_building(const QString &filename) {
  ...
  if (building.coordinate_system.is_global()) {
    ...
    scene->setSceneRect(QRectF(-M_PI * CoordinateSystem::WGS84_A,
...
void Editor::restore_previous_viewport() {
  ...
  if (previous_filename == building.get_filename()) {
    printf("restoring viewport: (%.1f, %.1f, %3f)\n", viewport_center_x,
           viewport_center_y, viewport_scale);
...
void Editor::building_new() {
  ...
  QSettings settings;
  settings.setValue(preferences_keys::previous_building_path,
...
void Editor::building_open() {
  ...
  QFileInfo file_info(file_dialog.selectedFiles().first());
  if (!file_info.exists()) {
    QMessageBox::critical(this, "File does not exist",
...
bool Editor::building_save() {
  if (!building.save()) {
    QMessageBox::critical(this, "Unable to save",
...
bool Editor::building_export_features() {
  ...
  auto result = building.export_features(
      level_idx, file_info.absoluteFilePath().toStdString());
  setWindowModified(false);
  return result;
}
...
void Editor::zoom_reset() { map_view->zoom_fit(level_idx); }
...
void Editor::mouse_event(const MouseType t, QMouseEvent *e) {
  ...
  if (level_idx >= static_cast<int>(building.levels.size())) {
    if (t == MOUSE_RELEASE) {
      if (building.get_filename().empty()) {
        ...
        QMessageBox::critical(this, "No levels defined",
  ...
  case TOOL_ADD_VERTEX:
    mouse_add_vertex(t, e, p);
    break;
  case TOOL_MOVE:
    mouse_move(t, e, p);
    break;
  case TOOL_ADD_LANE:
    mouse_add_lane(t, e, p);
    break;
  case TOOL_ADD_WALL:
    mouse_add_wall(t, e, p);
    break;
  case TOOL_ADD_MEAS:
    mouse_add_meas(t, e, p);
    break;
  case TOOL_ADD_DOOR:
    mouse_add_door(t, e, p);
    break;
  case TOOL_ADD_MODEL:
    mouse_add_model(t, e, p);
    break;
  case TOOL_ROTATE:
    mouse_rotate(t, e, p);
    break;
  case TOOL_ADD_FLOOR:
    mouse_add_floor(t, e, p);
    break;
  case TOOL_ADD_HOLE:
    mouse_add_hole(t, e, p);
    break;
  case TOOL_ADD_STORAGE_RACK:
    mouse_add_storage_rack(t, e, p);
    break;
  case TOOL_EDIT_POLYGON:
    mouse_edit_polygon(t, e, p);
    break;
  case TOOL_ADD_FEATURE:
    mouse_add_feature(t, e, p);
    break;
  case TOOL_ADD_CONSTRAINT:
    mouse_add_constraint(t, e, p);
    break;
  case TOOL_ADD_FIDUCIAL:
    mouse_add_fiducial(t, e, p);
    break;
  case TOOL_ADD_ROI:
    mouse_add_roi(t, e, p);
    break;
...
void Editor::mousePressEvent(QMouseEvent *e) { mouse_event(MOUSE_PRESS, e); }

void Editor::mouseReleaseEvent(QMouseEvent *e) {
  mouse_event(MOUSE_RELEASE, e);
}

void Editor::mouseMoveEvent(QMouseEvent *e) { mouse_event(MOUSE_MOVE, e); }
...
bool Editor::is_mouse_event_in_map(QMouseEvent *e, QPointF &p_scene) {
  ...
  if (p_map.x() < 0 || p_map.y() < 0 || p_map.x() >= map_view->width() ||
      p_map.y() >= map_view->height())
    return false;
...
void Editor::keyPressEvent(QKeyEvent *e) {
  ...
  case Qt::Key_1:
    ...
    break;
  case Qt::Key_2:
    number_key_pressed(2);
    break;
  case Qt::Key_3:
    number_key_pressed(3);
    break;
  case Qt::Key_4:
    number_key_pressed(4);
    break;
  case Qt::Key_5:
    number_key_pressed(5);
    break;
  case Qt::Key_6:
    number_key_pressed(6);
    break;
  case Qt::Key_7:
    number_key_pressed(7);
    break;
  case Qt::Key_8:
    number_key_pressed(8);
    break;
  case Qt::Key_9:
    number_key_pressed(9);
    break;
  default:
    break;
  }
...
void Editor::tool_toggled(int id, bool checked) {
  ...
  case TOOL_ADD_MEAS:
    ...
    break;
  case TOOL_ADD_FLOOR:
    statusBar()->showMessage("Left-click to add polygon vertices. "
      ...
      for (auto &em : editor_models) {
        if (em.name == model.model_name) {
          ...
          mouse_motion_model->setScale(
              ...
              building.levels[level_idx].drawing_meters_per_pixel);
          mouse_motion_model->setPos(previous_mouse_point.x(),
...
QTableWidgetItem *Editor::create_table_item(const QString &str, bool editable) {
...
void Editor::property_editor_set_row(const int row_idx, const QString &label,
                                     const QString &value,
                                     const bool editable) {
...
void Editor::property_editor_set_row(const int row_idx, const QString &label,
                                     const int &value, const bool editable) {
...
void Editor::property_editor_set_row(const int row_idx, const QString &label,
                                     ...
                                     const bool editable) {
  property_editor_set_row(row_idx, label,
...
void Editor::add_param_button_clicked() {
  ...
  if (object_type == "vertex") {
    ...
    AddPropertyCommand *cmd =
        new AddPropertyCommand(&building, dialog.get_param_name(),
                               Param(dialog.get_param_type()), level_idx);

    undo_stack.push(cmd);
    auto updated_id = cmd->get_vertex_updated();
    populate_property_editor(building.levels[level_idx].vertices[updated_id],
                             updated_id);
    setWindowModified(true);
  }
}
...
void Editor::delete_param_button_clicked() {
  QMessageBox::about(this, "work in progress",
...
void Editor::layer_edit_button_clicked(const int row_idx) {
  ...
  connect(dialog, &LayerDialog::redraw, [=]() {
    layer_table->update(building, level_idx, layer_idx);
    create_scene();
  });
  connect(dialog, &LayerDialog::center_layer, [=]() {
    dialog->set_center(map_view->get_center().x(), map_view->get_center().y());
  });
}
...
void Editor::populate_property_editor(const Edge &edge) {
  ...
  for (const auto &param : edge.params) {
    property_editor_set_row(row, QString::fromStdString(param.first),
...
void Editor::populate_property_editor(const Vertex &vertex, const int index) {
  ...
  if (!building.coordinate_system.is_global()) {
    ...
    if (ref_level_idx < building.levels.size()) {
      QPointF p_ref_level;
      building.transform_between_levels(level_idx, QPoint(vertex.x, vertex.y),
                                        ref_level_idx, p_ref_level);

      const double y_flip = building.coordinate_system.is_y_flipped() ? -1 : 1;
      const double scale =
          building.levels[ref_level_idx].drawing_meters_per_pixel;
      const QPointF p_meters(p_ref_level.x() * scale,
  ...
  property_editor_set_row(5, "name", QString::fromStdString(vertex.name), true);
...
void Editor::populate_property_editor(const Feature &feature) {
  ...
  property_editor_set_row(0, "name", QString::fromStdString(feature.name()),
                          true);
...
void Editor::populate_property_editor(const Fiducial &fiducial) {
  ...
  property_editor->setRowCount(1);
  property_editor_set_row(0, "name", QString::fromStdString(fiducial.name),
...
void Editor::populate_property_editor(const Model &model) {
  ...
  property_editor_set_row(0, "name",
                          QString::fromStdString(model.instance_name), true);

  property_editor_set_row(1, "model_name",
                          QString::fromStdString(model.model_name));

  property_editor_set_row(2, "elevation", model.state.z, 3, true);

  property_editor_set_row(
      3, "static", model.is_static ? QString("true") : QString("false"), true);

  property_editor_set_row(
      4, "dispensable",
      model.is_dispensable ? QString("true") : QString("false"), true);
...
void Editor::draw_mouse_motion_line_item(const double mouse_x,
...
void Editor::mouse_select(const MouseType type, QMouseEvent *e,
                          const QPointF &p) {
  if (type != MOUSE_PRESS)
    return;
  const QPoint p_global = mapToGlobal(e->pos());
  ...
  building.levels[level_idx].mouse_select_press(
      p.x(), p.y(), item, rendering_options, e->modifiers());
...
void Editor::mouse_add_vertex(const MouseType t, QMouseEvent *,
                              const QPointF &p) {
  if (t == MOUSE_PRESS) {
...
void Editor::mouse_add_feature(const MouseType t, QMouseEvent *,
                               const QPointF &p) {
  if (t == MOUSE_PRESS) {
    undo_stack.push(
        new AddFeatureCommand(&building, level_idx, layer_idx, p.x(), p.y()));
    setWindowModified(true);
    create_scene();
  }
}

void Editor::mouse_add_fiducial(const MouseType t, QMouseEvent *,
                                const QPointF &p) {
  if (t == MOUSE_PRESS) {
    AddFiducialCommand *command =
        new AddFiducialCommand(&building, level_idx, p.x(), p.y());
...
void Editor::mouse_move(const MouseType t, QMouseEvent *e, const QPointF &p) {
  if (t == MOUSE_PRESS) {
    Level::NearestItem ni =
        building.levels[level_idx].nearest_items(p.x(), p.y());

    printf("mouse press (%.3f, %.3f) feature_dist = %.3f, feature_idx = %d, "
           "feature_layer_idx = %d\n",
           p.x(), p.y(), ni.feature_dist, ni.feature_idx, ni.feature_layer_idx);

    // todo: use QGraphics stuff to see if we clicked a model pixmap...
    const double model_dist_thresh =
        0.5 / building.levels[level_idx].drawing_meters_per_pixel;

    if (ni.model_idx >= 0 && ni.model_dist < model_dist_thresh) {
      // Now we need to find the pixmap item for this model.
      const Model &model = building.levels[level_idx].models[ni.model_idx];
      mouse_motion_model =
          get_closest_pixmap_item(QPointF(model.state.x, model.state.y));
      mouse_model_idx = ni.model_idx;
      latest_move_model =
          new MoveModelCommand(&building, level_idx, mouse_model_idx);
    } else if (ni.vertex_idx >= 0 && ni.vertex_dist < 10.0) {
      mouse_vertex_idx = ni.vertex_idx;

      latest_move_vertex =
          new MoveVertexCommand(&building, level_idx, mouse_vertex_idx);
    } else if (ni.feature_idx >= 0 && ni.feature_layer_idx >= 0 &&
      ...
      latest_move_feature = new MoveFeatureCommand(
          &building, level_idx, ni.feature_layer_idx, ni.feature_idx);
    } else if (ni.fiducial_idx >= 0 && ni.fiducial_dist < 10.0) {
      mouse_fiducial_idx = ni.fiducial_idx;

      latest_move_fiducial =
          new MoveFiducialCommand(&building, level_idx, mouse_fiducial_idx);
    }
    ...
    */
    if (mouse_motion_model != nullptr) {
      ...
      Model &model = building.levels[level_idx].models[mouse_model_idx];
      model.state.x = p.x();
      model.state.y = p.y();
      ...
      printf("moved feature %d on layer %d to (%.1f, %.1f)\n",
             mouse_feature_idx, mouse_feature_layer_idx, feature->x(),
             feature->y());
      create_scene();
...
void Editor::mouse_add_edge(const MouseType t, QMouseEvent *e, const QPointF &p,
  ...
  if (t == MOUSE_PRESS) {
    ...
    if (prev_clicked_idx < 0) {
      latest_add_edge =
          new AddEdgeCommand(&building, level_idx, rendering_options);
      clicked_idx =
...
void Editor::mouse_add_constraint(const MouseType t, QMouseEvent * /*e*/,
                                  const QPointF &p) {
  ...
  if (t == MOUSE_PRESS) {
    ...
    if (!clicked_feature_id.isNull()) {
      ...
      AddConstraintCommand *command = new AddConstraintCommand(
          &building, level_idx, clicked_feature_id, f->id());
      undo_stack.push(command);
    ...
    QPen pen(QBrush(QColor::fromRgbF(0.8, 0.8, 0.4, 0.8)),
             0.1 / level->drawing_meters_per_pixel, Qt::SolidLine,
             Qt::RoundCap);

    if (!mouse_motion_line) {
      mouse_motion_line = scene->addLine(QLineF(feature_point, p), pen);
...
void Editor::mouse_add_lane(const MouseType t, QMouseEvent *e,
                            const QPointF &p) {
  mouse_add_edge(t, e, p, Edge::LANE);
}

void Editor::mouse_add_wall(const MouseType t, QMouseEvent *e,
                            const QPointF &p) {
  mouse_add_edge(t, e, p, Edge::WALL);
}

void Editor::mouse_add_meas(const MouseType t, QMouseEvent *e,
                            const QPointF &p) {
  mouse_add_edge(t, e, p, Edge::MEAS);
}

void Editor::mouse_add_door(const MouseType t, QMouseEvent *e,
                            const QPointF &p) {
  mouse_add_edge(t, e, p, Edge::DOOR);
}

void Editor::mouse_add_human_lane(const MouseType t, QMouseEvent *e,
                                  const QPointF &p) {
  mouse_add_edge(t, e, p, Edge::HUMAN_LANE);
}

void Editor::mouse_add_model(const MouseType t, QMouseEvent *,
                             const QPointF &p) {
  if (t == MOUSE_PRESS) {
    ...
    AddModelCommand *cmd = new AddModelCommand(
        &building, level_idx, p.x(), p.y(), mouse_motion_editor_model->name);
    undo_stack.push(cmd);
    setWindowModified(true);
    create_scene();
  } else if (t == MOUSE_MOVE) {
...
void Editor::mouse_rotate(const MouseType t, QMouseEvent *mouse_event,
                          const QPointF &p) {
  if (t == MOUSE_PRESS) {
    clicked_idx = building.nearest_item_index_if_within_distance(
        level_idx, p.x(), p.y(), 50.0, Level::MODEL);
    if (clicked_idx < 0)
      return; // nothing to do. click wasn't on a model.

    latest_rotate_model =
        new RotateModelCommand(&building, level_idx, clicked_idx);
    ...
    mouse_motion_ellipse =
        scene->addEllipse(model.state.x - r, // ellipse upper-left column
                          model.state.y - r, // ellipse upper-left row
                          2 * r,             // ellipse width
                          2 * r,             // ellipse height
                          pen);
    mouse_motion_line = scene->addLine(
        model.state.x, model.state.y, model.state.x + r * cos(model.state.yaw),
        model.state.y - r * sin(model.state.yaw), pen);
  } else if (t == MOUSE_RELEASE) {
    // remove_mouse_motion_item();
    if (clicked_idx < 0)
      return;
    ...
    if (mouse_event->modifiers() & Qt::ShiftModifier)
      mouse_yaw = discretize_angle(mouse_yaw);
    const double r = static_cast<double>(ROTATION_INDICATOR_RADIUS);
    mouse_motion_line->setLine(model.state.x, model.state.y,
                               model.state.x + r * cos(mouse_yaw),
                               model.state.y - r * sin(mouse_yaw));

    if (mouse_motion_model)
      mouse_motion_model->setRotation((-mouse_yaw + M_PI / 2.0) * 180.0 / M_PI);
  }
}
...
void Editor::mouse_add_polygon(const MouseType t, QMouseEvent *e,
                               const QPointF &p,
    ...
    if (e->buttons() & Qt::LeftButton) {
      ...
      if (mouse_motion_polygon == nullptr) {
        ...
        mouse_motion_polygon = scene->addPolygon(
            polygon, pen, QBrush(QColor::fromRgbF(1.0, 0.0, 0.0, 0.5)));
        mouse_motion_polygon_vertices.clear();
      }

      // only add vertex_idx is NOT already in the vertex list
      if (std::find(mouse_motion_polygon_vertices.begin(),
      ...
      if (mouse_motion_polygon_vertices.size() >= 3) {
        ...
        AddPolygonCommand *command =
            new AddPolygonCommand(&building, polygon, level_idx);
...
void Editor::mouse_add_floor(const MouseType t, QMouseEvent *e,
                             const QPointF &p) {
  mouse_add_polygon(t, e, p, Polygon::FLOOR);
}

void Editor::mouse_add_hole(const MouseType t, QMouseEvent *e,
                            const QPointF &p) {
...
void Editor::update_storage_racks() {
  Level *level = active_level();
  if (level == nullptr)
    return;
  level->update_storage_racks();
  printf("storage_racks_updated()\n");
}


void Editor::mouse_edit_polygon(const MouseType t, QMouseEvent *e,
                                const QPointF &p) {
  ...
  if (t == MOUSE_PRESS) {
    if (e->buttons() & Qt::RightButton) {
      ...
      PolygonRemoveVertCommand *command =
          new PolygonRemoveVertCommand(selected_polygon, ni.vertex_idx);
      undo_stack.push(command);
      setWindowModified(true);
      create_scene();
    } else if (e->buttons() & Qt::LeftButton) {
      mouse_edge_drag_polygon = building.polygon_edge_drag_press(
          level_idx, selected_polygon, p.x(), p.y());
      ...
      mouse_motion_polygon =
          scene->addPolygon(mouse_edge_drag_polygon.polygon, QPen(Qt::black),
    ...
    if (std::find(selected_polygon->vertices.begin(),
    ...
    PolygonAddVertCommand *command = new PolygonAddVertCommand(
        selected_polygon, mouse_edge_drag_polygon.movable_vertex,
        release_vertex_idx);
...
bool Editor::maybe_save() {
  ...
  const QMessageBox::StandardButton button_clicked = QMessageBox::warning(
      this, "Building not saved!", "Do you want to save your changes?",
...
void Editor::showEvent(QShowEvent *event) { QMainWindow::showEvent(event); }
...
void Editor::closeEvent(QCloseEvent *event) {
  ...
  printf("closeEvent:  (%d, %d) -> (%.1f, %.1f)\n", p_center_window.x(),
         p_center_window.y(), p_center_scene.x(), p_center_scene.y());
  ...
  if (!building.levels.empty())
    settings.setValue(preferences_keys::level_name,
...
void Editor::clear_current_tool_buffer() {
  if (tool_id == TOOL_ADD_WALL || tool_id == TOOL_ADD_LANE ||
      tool_id == TOOL_ADD_MEAS || tool_id == TOOL_ADD_HUMAN_LANE ||
