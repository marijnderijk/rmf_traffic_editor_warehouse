...
public:
  ...
  Polygon::EdgeDragPolygon polygon_edge_drag_press(const Polygon *polygon,
                                                   const double x,
                                                   const double y);
  ...
  NearestItem nearest_items(const double x, const double y);

  int nearest_item_index_if_within_distance(const double x, const double y,
                                            const double distance_threshold,
                                            const ItemType item_type);

  void mouse_select_press(const double x, const double y,
  ...
  void set_selected_containing_polygon(const double x, const double y);

  void draw(QGraphicsScene *scene, std::vector<EditorModel> &editor_models,
            const RenderingOptions &rendering_options,
...
private:
  double point_to_line_segment_distance(const double x, const double y,
                                        const double x0, const double y0,
                                        const double x1, const double y1,
                                        double &x_proj, double &y_proj);

  void load_yaml_edge_sequence(const YAML::Node &data,
                               const char *sequence_name,
                               const Edge::Type type);

  bool _drawing_visible = true;

  void draw_lane(QGraphicsScene *scene, const Edge &edge,
                 const RenderingOptions &rendering_options,
  ...
  void draw_constraint(QGraphicsScene *scene, const Constraint &constraint,
                       int constraint_idx) const;

  // helper function
  void draw_polygon(QGraphicsScene *scene, const QBrush &brush,
                    const Polygon &polygon) const;

  void add_door_swing_path(QPainterPath &path, double hinge_x, double hinge_y,
                           double door_length, double start_angle,
                           double end_angle) const;

  void add_door_slide_path(QPainterPath &path, double hinge_x, double hinge_y,
                           double door_length, double door_angle) const;
};
